...
from Flight_Offers_Search import get_flight_offers_via_query
from Flight_inspiration_search import get_user_input_with_llm
...
def classify_api_query_with_llm(user_input):
    ...
    Topics:
    1. Topic 1 answers the question "What are the flight available between 2 cities"?
    2. Topic 2 answers the question "What are the cheapest destinations to fly from a city"?
    3. Topic 3 answers the question "What are the cheapest flight between 2 cities?"
...
def main():
    ...
    while True:
        ...
        if query_type == "- rag-related":
            #print("entered Rag section")
            prompt = create_prompt(user_input)
            #print(prompt)
            reply = rag_agent.generate_reply(messages=[{"content": prompt, "role": "user"}])
            print(reply)
        elif query_type == "- API-Related":
            response = classify_api_query_with_llm(user_input)
            if response == '1':
                reply = handle_flight_query_with_llm(user_input)
                print(reply)
            if response == "2":
                reply = get_user_input_with_llm(user_input)
                print(reply)
            if response == "3":
                reply = get_flight_offers_via_query(user_input)
                print(reply)
        else:
